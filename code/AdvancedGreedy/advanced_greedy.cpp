    ...
    void build(const Graph& g, const vector<int>& seeds) {
        ...
        for (int u = 0; u < g.n; u++) {
            for (auto& [v, p] : g.adj[u]) {
                if (seedSet.count(v)) continue; // skip edges into seed nodes
                if (seedSet.count(u)) {
...
public:
    ...
    void build(const SampledGraph& sg, int root_node) {
        ...
        for (int i = dfs_counter; i >= 2; i--) {
            ...
            for (int v : radj_lt[w]) {
                if (dfn[v] == 0 && v != root) continue; // not visited
                int u = eval(v);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (string(argv[i]) == "-dataset") dataset = argv[++i];
        else if (string(argv[i]) == "-k") budget = atoi(argv[++i]);
        ...
        else if (string(argv[i]) == "-algo") algo = argv[++i];
        else if (string(argv[i]) == "-timeLimit") time_limit = stod(argv[++i]);
        else if (string(argv[i]) == "-seedFile") seedFile = argv[++i];
        else if (string(argv[i]) == "-output") outputFile = argv[++i];
    }
